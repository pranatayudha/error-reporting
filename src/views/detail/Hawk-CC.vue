...
>
    <v-toolbar
        ...
        class="black--text"
    >
        <v-subheader> List issues of Hawk City Councils</v-subheader>
    ...
    <v-list three-line class="py-0">
        <v-list-group
        v-for="item in HawkCC" :key="item.id"
        ...
        <v-card elevation="0" outlined min-width="100%" max-width="100%" v-if="!loadClick">
            <v-card-text class="mr-3 pl-2">
              ...
              <div class="text-lg-left"><b>Created At : </b> {{ date(item.created_at) }}</div>
              ...
              <!-- if list are hawk cc -->
              <!-- <div class="text-lg-left" v-if="item.project == 'Hawk-CC' && item.content_obj.info"><b>File : </b> {{item.content_obj.info.context.file}}</div>
              <div class="text-lg-left" v-if="item.project == 'Hawk-CC' && item.content_obj.info"><b>Line : </b> {{item.content_obj.info.context.line}}</div>
              <div class="text-lg-left" v-if="item.project == 'Hawk-CC' && item.content_obj.info"><b>Code : </b> {{item.content_obj.info.context.code}}</div>
              <div class="text-lg-left" v-if="item.project == 'Hawk-CC' && item.content_obj.info">
                <b>Message : </b>{{item.content_obj.info.context.message}}
              </div> -->
    ...
    <v-alert class="mx-3 mb-3" v-if="!HawkCC.length" type="error">
        There's no data
    </v-alert>
</v-card>
      ...
      HawkCC () {
        ...
        if (!this.search && !this.sortByStatus && !this.sortByDate) {
          ...
          if(this.sortByStatus === 'Open'){
            return this.listHawkCC.filter(item => item.status == 0);
          } else if(this.sortByStatus === 'Pending'){
            return this.listHawkCC.filter(item => item.status == 1)
          } else if(this.sortByStatus === 'Checking'){
            return this.listHawkCC.filter(item => item.status == 2)
          } else if(this.sortByStatus === 'Closed'){
            return this.listHawkCC.filter(item => item.status == 3)
          } else {
            return this.listHawkCC
          }
        } else if(this.search && this.sortByStatus){
          const search = this.search.toLowerCase();
          const searchFunc = this.listHawkCC.filter(item => {
            let matchContent = false
            const matchTitle =  item.title.toLowerCase().indexOf(search) > -1
            const matchID =  item.project+'#'+item.id
            const matchName = matchID.toString().toLowerCase().indexOf(search) > -1
          ...
          if(this.sortByStatus === 'Open' && this.sortByDate === 'Newest'){
            const data = this.listHawkCC.filter(item => item.status == 0);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkCC.filter(item => item.status == 1);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkCC.filter(item => item.status == 2);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkCC.filter(item => item.status == 3);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
