    ...
    HawkID() {
      ...
      if (!this.search && !this.sortByStatus) {
        ...
        if (this.sortByStatus === "Open") {
          ...
          return this.listHawkID.filter((item) => item.status == 1);
        } else if (this.sortByStatus === "Checking") {
          return this.listHawkID.filter((item) => item.status == 2);
        } else {
          return this.listHawkID;
        }
      } else if (this.search && this.sortByStatus) {
        const search = this.search.toLowerCase();
        const searchFun = this.listHawkID.filter((item) => {
          const matchID = item.id;
          const matchName =
            matchID
              .toString()
