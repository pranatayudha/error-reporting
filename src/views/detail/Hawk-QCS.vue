...
>
    <v-toolbar
        ...
        class="black--text"
    >
        <v-subheader> List issues of Hawk Queensland Corrective Services</v-subheader>
    ...
    <v-list three-line class="py-0">
        <v-list-group
        v-for="item in HawkQCS" :key="item.id"
    ...
    <v-alert class="mx-3 mb-3" v-if="!HawkQCS.length" type="error">
        There's no data
    </v-alert>
</v-card>
      ...
      HawkQCS () {
        ...
        if (!this.search && !this.sortByStatus && !this.sortByDate) {
          ...
          if(this.sortByStatus === 'Open'){
            return this.listHawkQCS.filter(item => item.status == 0);
          } else if(this.sortByStatus === 'Pending'){
            return this.listHawkQCS.filter(item => item.status == 1)
          } else if(this.sortByStatus === 'Checking'){
            return this.listHawkQCS.filter(item => item.status == 2)
          } else if(this.sortByStatus === 'Closed'){
            return this.listHawkQCS.filter(item => item.status == 3)
          } else {
            return this.listHawkQCS
          }
        } else if(this.search && this.sortByStatus){
          const search = this.search.toLowerCase();
          const searchFunc = this.listHawkQCS.filter(item => {
            let matchContent = false
            const matchTitle =  item.title.toLowerCase().indexOf(search) > -1
            const matchID =  item.project+'#'+item.id
            const matchName = matchID.toString().toLowerCase().indexOf(search) > -1
          ...
          if(this.sortByStatus === 'Open' && this.sortByDate === 'Newest'){
            const data = this.listHawkQCS.filter(item => item.status == 0);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkQCS.filter(item => item.status == 1);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkQCS.filter(item => item.status == 2);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
            ...
            const data = this.listHawkQCS.filter(item => item.status == 3);
            data.sort((a,b) => {
              var key1 = new Date(a.created_at);
              var key2 = new Date(b.created_at);
              if (key2 < key1) {
